...
public:
	...
	T& operator() ( int index1, int index2) { return array_[index1,index2]; }

	const T& operator() (int index1, int index2) const { return array_[index1,index2]; }
#else 
	class ArrayID
	{
	public:
...
public:
	...
	void remove_reference() { if(0 == --ref_count_) delete this; }

	void mark_unshareable() { shareable_ = false; }

	bool shareable() const { return shareable_; }

	bool is_shared() const { return ref_count_ > 1; }
private:
	int ref_count_;
...
{
public:
	RCPtr(_Tp* real_ptr = 0) : pointee_(real_ptr) { _init(); }  

	RCPtr(const RCPtr& rhs) : pointee_(rhs.pointee_) { _init(); }  

	~RCPtr() { if(pointee_) pointee_->remove_reference(); }
	...
	_Tp* operator-> () const { return pointee_; }
...
private:
	struct StringValue : public RCObject
	{
		...
		StringValue(const StringValue& rhs) { _init(rhs.data_); }
