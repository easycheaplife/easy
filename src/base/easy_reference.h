...
namespace easy
{
	...
	public:
		Reference() { ref_to_ = easy_null; ref_from_ = easy_null; }
		virtual ~Reference() {}
		...
		void UnLink() { TargetObjectDestroyLink(); DeLink(); ref_to_ = easy_null; ref_from_ = easy_null; }

		// Link is invalid due to destruction of referenced target object. Call comes from the refTo object
		// Tell our refFrom object, that the link is cut
		void Invalidate()                                   // the iRefFrom MUST remain!!
		{
			SourceObjectDestroyLink(); DeLink(); ref_to_ = easy_null;
		}
		...
		TO* operator ->() const { return ref_to_; }
		TO* GetTarget() const { return ref_to_; }

		FROM* GetSource() const { return ref_from_; }
	};
}
