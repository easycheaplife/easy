...
namespace easy
{
	...
	{
	public:
		EasyRefCountPtr(T* real_ptr = 0) : pointee_( real_ptr ){ _init(); }

		EasyRefCountPtr(const EasyRefCountPtr& rhs) : pointee_( rhs.pointee_) { _init(); }

		~EasyRefCountPtr() { if (pointee_) { pointee_->RemoveReference();} } 
