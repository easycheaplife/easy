...
struct _Refcount_Base {
    ...
    _Refcount_Base(_RC_t __n) : ref_count_(__n) {
        pthread_mutex_init(&ref_count_lock_, 0);
    }
#else
    _Refcount_Base(_RC_t __n) : ref_count_(__n) { }
    ...
    void incr()	{
        ::InterlockedIncrement((_RC_t*)&ref_count_);
    }
    _RC_t decr() {
        return ::InterlockedDecrement((_RC_t*)&ref_count_);
    }
#else	//	no use thread
    void incr()	{  }
    _RC_t decr() {  }
...
#pragma warning(suppress: 6031)
    void initialize() {
        if (0) {
            InitializeCriticalSectionAndSpinCount(&lock_,0x000004000);
        } else {
...
struct auto_lock {
    mutex_lock&	lock_;
    auto_lock(mutex_lock& __lock) : lock_(__lock) {
        lock_.acquire_lock();
    }
    ~auto_lock() {
        lock_.release_lock();
...
struct rw_lock {
    ...
    void initialize()   {
        r_cnt_ = 0;
        w_cnt_ = 0;
        pthread_mutex_init(&cont_lock_, NULL);
        pthread_cond_init(&cont_var_, NULL);
    }
    int acquire_r_lock() {
    ...
    void uninitialize() {
        pthread_mutex_destroy(&cont_lock_);
        pthread_cond_destroy(&cont_var_);
    }
#else
